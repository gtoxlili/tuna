...
"版本号即发版决定"上线一天即回滚:普通代码推送完全不触发 CI,与维护者"推了就该发"的
工作流相抵。现方案合并两代的优点:每次触及构建输入的推送自动发版(patch = 提交计数),
major.minor 取自 Cargo.toml(升大版本改它);防 clobber 守卫、Cargo.lock 一致性检查、
容器构建、tap dispatch 全部保留;`tuna --version` 恢复 TUNA_VERSION 编译期注入以对齐
自动 tag。
