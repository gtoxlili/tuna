...
- **AI 对话**。新词揭示前按 `a` 和 AI 多轮对话着拆词：你说你看到了哪些词素，它拿着已核验的词素引导你自己推出词义，绝不直接说答案。揭示后按 `a` 是易混词辨析对话，AI 先手给出对照引导。回复可以朗读出来（中英混合语音，对话内 `Tab` 切换，同样只走绑定耳机）。需要 DeepSeek 密钥，学习本身离线可用。
...
学习时的按键：`Tab` 打开命令菜单，`Enter` 揭示答案，`Space` 发音，`a` AI 对话（揭示前推导、揭示后辨析），`w` 词源，`g` 星座，`s` 设置，`?` 帮助，`u` 撤销上一次评分，`Esc` 退出。
